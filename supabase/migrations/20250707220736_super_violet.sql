/*
  # Complete Luminara Database - Missing Functions, Triggers & Storage
  
  This migration adds the missing critical components:
  1. Storage buckets for user files
  2. Additional functions for learning analytics
  3. Advanced triggers for real-time updates
  4. Prompt selection algorithms
  5. Achievement checking system
  6. Data cleanup and maintenance functions
*/
...
CREATE INDEX IF NOT EXISTS idx_problem_submissions_date ON public.problem_submissions(DATE(created_at));
CREATE INDEX IF NOT EXISTS idx_problem_submissions_user_date ON public.problem_submissions(user_id, DATE(created_at));
